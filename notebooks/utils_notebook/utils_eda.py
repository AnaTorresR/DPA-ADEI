...
def changeType_date(df, cols, type_format):
    for i in cols:
        df[i] = pd.to_datetime(df[i].str.strip(), format=type_format)
    return


def checkDateForm(df, cols, date_format):
    
    """
    Check date column format.
     args:
         df (DataFrame): DataFrame with the data.
         cols (list): list with the name of columns by date
         date_format(str): format of date dd/mm/aaaa
     returns:
          Info(print): Number of data with the format or not.
    
    """
    date_exp = ""
    if date_format == "mm/dd/aaaa":
        date_exp = '^(0[1-9]|1[0-2])(\/|-)([0-2][0-9]|3[0-1])(\/|-)(\d{4})$'
        print(date_format)
    elif date_format == "dd/mm/aaaa":
        date_exp = '^([0-2][0-9]|3[0-1])(\/|-)(0[1-9]|1[0-2])(\/|-)(\d{4})$'
    
    for i in cols:
        che = df[df[i].str.match(date_exp) == False]
        s = pd.Series(che[i].value_counts().index).str.extract(r'(\d{2})$')[0].value_counts()
        print("\nCantidad de valores en " + i + " que no cumplen con el formato {} : {}".format(date_format, len(che)))
